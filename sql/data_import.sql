-- Cell information
CREATE TABLE cells (
    cell_id TEXT PRIMARY KEY,
    province_name TEXT,
    district_name TEXT,
    sector_name TEXT,
    cell_name TEXT
);

-- Population 
CREATE TABLE pop (
    id SERIAL PRIMARY KEY,
    cell_id TEXT REFERENCES cells(cell_id),
    elderly_60 REAL,
    general_pop REAL,
    children_under5 REAL,
    youth_15_24 REAL,
    men_2020 REAL,
    women_2020 REAL,
    building_count REAL
);

-- Annual NTL stats
CREATE TABLE ntl_annual (
    id SERIAL PRIMARY KEY,
    cell_id TEXT REFERENCES cells(cell_id),
    ntl_min REAL,
    ntl_max REAL,
    ntl_mean REAL,
    ntl_median REAL,
    ntl_sum REAL,
    pixel_count REAL,
    raster_filename TEXT,
    year INT
);


-- Monthly NTL stats
CREATE TABLE ntl_monthly (
    id SERIAL PRIMARY KEY,
    cell_id TEXT REFERENCES cells(cell_id),
    ntl_min REAL,
    ntl_max REAL,
    ntl_mean REAL,
    ntl_median REAL,
    ntl_sum REAL,
    pixel_count REAL,
    raster_filename TEXT,
    year INT,
    month INT,
    date DATE
);


-- LOAD DATA FROM CSV FILES
COPY cells FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/cells.csv' DELIMITER ',' CSV HEADER;

COPY pop(cell_id, elderly_60, children_under5, youth_15_24, general_pop, men_2020, women_2020, building_count)
FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/population.csv'
CSV HEADER;

COPY ntl_annual (cell_id, ntl_min, ntl_max, ntl_mean, ntl_median, ntl_sum,
                  pixel_count, raster_filename, year)
FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/ntl-annual-2012-2024.csv' 
DELIMITER ',' CSV HEADER;

\COPY ntl_monthly(cell_id, ntl_min, ntl_max, ntl_mean, ntl_median, ntl_sum,
                  pixel_count, raster_filename, year, month, date)
FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/ntl-monthly-2012-2024.csv'
CSV HEADER;

COPY ntl_annual (cell_id, ntl_min, ntl_max, ntl_mean, ntl_median, ntl_sum,
                  pixel_count, raster_filename, year)
FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/ntl-annual-2012-2024.csv' 
DELIMITER ',' CSV HEADER;

\COPY ntl_monthly(cell_id, ntl_min, ntl_max, ntl_mean, ntl_median, ntl_sum,
                  pixel_count, raster_filename, year, month, date)
FROM '/home/ira2351696/Desktop/AIMS-DSCBI/data/tmp-db-data/ntl-monthly-2012-2024.csv'
CSV HEADER;
